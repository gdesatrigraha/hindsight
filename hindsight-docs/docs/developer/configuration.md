...
| `HINDSIGHT_API_OBSERVATION_SCOPE_TAG_KEY_WHITELIST` | JSON array of tag keys permitted to participate in newly generated tagged observation scopes. Fact tags remain unchanged. Unset preserves legacy behavior; `[]` permits no tagged dimensions; no matching tags falls back to the shared scope `[[]]`. Configurable per bank. | - |
...
1. **Configurable Fields** - Safe behavioral settings that can be customized per-bank:
   ...
   - Observations: `enable_observations`, `enable_auto_consolidation`, `observations_mission`, `max_observations_per_scope`, `observation_scope_tag_key_whitelist`
