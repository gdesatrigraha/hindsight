...
| `observationScopesParam` | —                                    | file-only server-side parameters for scope generation; `{"tagKeyWhitelist": ["project"]}` keeps provenance tags on facts while allowing only `project:*` tags to define observation boundaries                                                                                                                                                                                                                                                    |
...
When a shared bank contains several repositories, keep all provenance tags on the facts while
restricting which dimensions define observations. The server applies the whitelist first and then
uses the selected `observationScopes` strategy:
...
For example, a fact tagged `project:example`, `source:chat`, and `harness:codex` keeps all three
tags for recall and provenance, while only `project:example` participates in the combined observation
scope. The same parameter composes with `per_tag` and `all_combinations`; an explicitly empty
whitelist or one with no matching tags falls back to the shared untagged scope. This is a server
feature, so changing it affects new Retain operations and does not rewrite historical observations.
`observationScopesParam` is file-only; environment variables can select a scalar
`observationScopes` mode but cannot configure the whitelist.
