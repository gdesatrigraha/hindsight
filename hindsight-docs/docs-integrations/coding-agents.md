...
When a shared bank contains several repositories, `retainTags` can keep project provenance on every
fact while the bank's observation-scope tag-key whitelist decides which tag dimensions may define
observations. Configure that policy on the bank through Hindsight's bank configuration API or
Control Plane. The coding-agent integration continues to send the complete provenance tags and the
selected `observationScopes` strategy; it does not own or overwrite the bank policy.
